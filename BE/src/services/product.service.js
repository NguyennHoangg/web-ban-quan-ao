const { findManyForList: findManyForListModel, findByIdDetailBySlug } = require("../model/product.model");
...
const findProductDetailsBySlug = async (slug) => {
  try {
    if(!slug) {
      throw createError(VALIDATION_ERRORS.MISSING_REQUIRED_FIELD, "Slug là bắt buộc");
    }
    const result = await findByIdDetailBySlug(slug);
    if(!result){
      throw createError(PRODUCT_ERRORS.PRODUCT_NOT_FOUND, "Không tìm thấy sản phẩm với slug đã cho");
    }
    return result;
  } catch (error) {
    throw error;
